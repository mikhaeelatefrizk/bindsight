...
affinity validation) is templated to free Google Colab notebooks, paid Modal
...
The GPU half — design with `RFdiffusion` + `ProteinMPNN` + `Boltz-2` — is
delivered as a `bindsight design --backend colab` command that produces a
self-contained Jupyter notebook with proven install + inference cells
patterned on the canonical upstream notebooks
(`ColabDesign`, `dl_binder_design` [@Bennett2023]).
...
The package ships **175 unit and integration tests** that run in under
four minutes and cover: the Pydantic v2 manifest schema, every API client
