...
all became true between late 2024 and 2026, and not before:
...
\begin{enumerate}
  ...
  \item \textbf{Permissive licensing} for state-of-the-art structure-and-
        ...
        both code and weights under MIT and Chai-1r under Apache-2.0, where
        the field's most capable predictor, AlphaFold~3, restricts commercial
        use of its weights.
...
We present \texttt{bindsight} (v0.3.6), a Python package with a
...
The \texttt{bindsight} release ships \textbf{2,000+ unit and
...
component states a permissive licence (MIT / Apache-2.0 / BSD / CC0 / CC-BY),
except the SURFY surfaceome list, whose source states none; manuscripts, figures
and generated results under \texttt{paper/} are CC BY 4.0.
Optional opt-in components with non-commercial restrictions
(notably PyRosetta, on which the \texttt{AF2-IG} validator path depends) are
...
\paragraph{Limitations of v0.3.6.} The CPU half of the pipeline
