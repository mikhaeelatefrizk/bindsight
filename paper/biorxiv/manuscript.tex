...
ProteinMPNN sequence $\to$ Boltz-2 validation) executes end-to-end on free
Google Colab notebooks, paid Modal serverless GPU, local NVIDIA Docker, or
Kaggle. A public web demo at
\href{https://bindsight.streamlit.app}{bindsight.streamlit.app}
runs the full discovery pipeline in any browser without installation. On a real
...
GPU stages run end-to-end on free Colab, paid Modal, local Docker, or Kaggle. All
...
shown below (verbatim from the run, abbreviated):
...
a small random cohort they need not rank in the top-$k$ by fold-change ---
...
local installation; the first visitor on a fresh container triggers the
download + analysis and the result is cached for subsequent visitors.

% --------------------------------------------------------------------
\section{Quality assurance and reproducibility}

The \texttt{bindsight} release ships \textbf{200+ unit and
...
\paragraph{Rediscovery validation.} A companion validation report
(\texttt{paper/validation/manuscript.md}, with reproducible artifacts under
\texttt{benchmarks/validation/}) tests the discovery half on six real
indication-matched TCGA tumor-vs-adjacent-normal cohorts. Scoring the rank of
each known antigen in the unsupervised candidate shortlist, the pipeline
rediscovers \textbf{ERBB2 (HER2) at rank~4} in HER2-enriched breast cancer
(log2fc~4.4, $p_{\mathrm{adj}}\!=\!1.7\times10^{-59}$) --- a result that depends
on stratifying TCGA-BRCA by its PAM50 subtype, which otherwise averages the HER2
signal away. The pipeline is also \textit{specific}: antigens that are not
transcriptionally over-expressed at the bulk level --- EGFR (mutation/
amplification-driven) and CEA (co-expressed in normal colon) --- are correctly
left out of the top-20, so sensitivity tracks differential-expression effect
size as expected for a DE-based method. Antigens without matched TCGA normals
(CLDN6, CD33/CD123) are documented as data limitations rather than reported with
a manufactured number. A complementary, CPU-tested three-way \textit{designer
benchmark} (RFdiffusion+ProteinMPNN vs BindCraft vs BoltzGen) ships as a runnable
harness and protocol in \texttt{benchmarks/designer\_benchmark/}, to be populated
from a GPU run.
