...
**The evidence, no waiting:** **[Real results](https://mikhaeelatefrizk.github.io/bindsight/results/)** — a fifteen-cohort rediscovery study on real TCGA RNA-seq, with CA9 surfaced at rank 1 of 291 in clear-cell kidney, and 20 de novo ERBB2 binders rendered in 3-D against their target. Static, always current with `main`, nothing to wake up.
...
> 🚀 **v0.2.2** — discovery half end-to-end on CPU (real TCGA data); design + validation demonstrated end-to-end on a **free GPU** — 20 de novo ERBB2 binders, best ipTM 0.88, 40% success@0.65 (8/20, 95% CI 15–70% over backbones), with the real Boltz-2-predicted complexes, in the [designer benchmark](benchmarks/designer_benchmark/RESULTS.md); web UI deployed as a Hugging Face Space.
...
> ⚠️ **The success rate below is withdrawn as a measure of design quality.** A paired control folded each design alongside a shuffle of its own sequence. Under a seeded validator averaging five diffusion draws, the shuffles pass more often than the designs: 50% of shuffles clear 0.65 against 30% of designs, a paired difference of -0.043 (95% CI -0.142 to +0.054, exact sign-flip p = 0.40), with 9 of 20 designs beating their own shuffle where 10 is chance. The ipTM values are real Boltz-2 outputs — what is withdrawn is the claim that the rate measures the designs. See [the calibration](benchmarks/calibration/README.md).
...
| **RFdiffusion, ProteinMPNN, Boltz-2** | Verified on free hardware | The committed benchmark ran the whole stack on a free Kaggle T4, from a wheel built out of this tree, under the corrected ProteinMPNN protocol. All 20 designs carry a target chain byte-identical to the native domain IV, so "the target was held fixed" is a check rather than a claim. |
...
- ✅ **Rediscovery study** — the discovery half, run on **fifteen whole, unstratified TCGA projects** as patient-paired contrasts and scored against a pre-registered panel of 22 antigen-cohort pairs. Nothing in a cohort's definition refers to the antigen being sought. Recall at rank 20 is **1 of 17** on approved-agent antigens (95% CI 0.01–0.27), and 3 of 22 across every tier. Five antigens surface, led by **CA9 at rank 1 of 291** in clear-cell kidney. The study reports four separate reasons an antigen can fail to appear rather than one rate, and that separation found two real defects in the pipeline, both since fixed. Artifacts in [`benchmarks/study/`](benchmarks/study/RESULTS.md); write-up in [`paper/validation/`](paper/validation/manuscript.md).
  - ⚠️ **This replaces an earlier claim of ERBB2 at rank 4 and recall@5 of 33%, which is withdrawn.** That breast cohort was stratified by a PAM50 subtype call, and ERBB2 is one of the fifty genes the PAM50 classifier is built on, so the tumour arm was selected partly by expression of the gene then reported as discovered.
- ✅ **De novo binder design demonstrated end-to-end** — the design half (RFdiffusion → ProteinMPNN → Boltz-2) run on a **free Kaggle Tesla T4** produced **20 real binders** against the ERBB2 extracellular **domain IV** (the clinically validated trastuzumab epitope): mean **ipTM 0.51**, best **0.88**, **40 %** of designs pass the ipTM ≥ 0.65 success bar (mean PAE-interaction 15.6 Å) — at **$0** in 0.72 GPU-hours, no local GPU. The target chain is held fixed, and that is checked rather than claimed: all 20 designs carry a chain byte-identical to the native 142-residue domain IV. **An earlier P100 run reporting mean 0.59, best 0.84 and 50 % is withdrawn** — it invoked ProteinMPNN without `--pdb_path_chains`, so it redesigned the target chain along with the binder. The real Boltz-2-predicted **complexes** (CIF) + FASTAs + per-design metrics are in [`benchmarks/designer_benchmark/RESULTS.md`](benchmarks/designer_benchmark/RESULTS.md); reproduce on a free GPU via [`RUN_FREE_GPU.md`](benchmarks/designer_benchmark/RUN_FREE_GPU.md).
