...
def _bundled_uniprot_set() -> frozenset[str]:
    """Return the small offline-test surface-protein set bundled with xpr2bind."""
    try:
        # Optional bundled list — fall back to the hardcoded set if missing.
        text = (
            resources.files("xpr2bind.surfaceome").joinpath("data", "surfy_offline.txt").read_text()
        )
        return frozenset(
...
def load_surfy(*, allow_offline_fallback: bool = True) -> frozenset[str]:
    ...
    1. ``<user_cache>/xpr2bind/surfy/surfy_v1.uniprot.txt`` (full canonical list,
    ...
    See ``xpr2bind/surfaceome/data/surfy_offline.txt`` for the fallback list.
