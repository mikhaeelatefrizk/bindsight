# What is xpr2bind, and why does it matter?
...
**`xpr2bind` is the first open-source tool that takes RNA-seq counts as input
...
`xpr2bind` does it in a single command, on a CPU laptop, with reproducibility
...
Anyone with a GPU can run the existing tools. The work `xpr2bind` does is the
