...
def build_kernel_script(
    *,
    ...
    header = "\n".join(
        [
            "# Auto-generated by bindsight KaggleRunner — split-env design+validation.",
            f"HANDLE_ID = {json.dumps(handle_id)}",
            f"PAYLOAD = {json.dumps(payload)}",
            f"N_TRAJ_NOTE = {json.dumps(n_trajectories_note)}",
            f"RFDIFF_REPO = {json.dumps(tools.RFDIFF_REPO)}",
            f"RFDIFF_COMMIT = {json.dumps(tools.RFDIFF_COMMIT)}",
            f"RFDIFF_WEIGHTS = {json.dumps(tools.RFDIFF_WEIGHTS)}",
            f"MPNN_REPO = {json.dumps(tools.PROTEINMPNN_REPO)}",
            f"MPNN_COMMIT = {json.dumps(tools.PROTEINMPNN_COMMIT)}",
            f"BOLTZ_PIP = {json.dumps(tools.BOLTZ_PIP)}",
            f"SE3_TORCH = {json.dumps(_SE3_TORCH)}",
            f"SE3_TORCH_INDEX = {json.dumps(_SE3_TORCH_INDEX)}",
            f"SE3_DGL = {json.dumps(_SE3_DGL)}",
            f"SE3_DGL_FIND = {json.dumps(_SE3_DGL_FIND)}",
            f"SE3_EXTRA_PIP = {json.dumps(_SE3_EXTRA_PIP)}",
            f"BOLTZ_TORCH = {json.dumps(_BOLTZ_TORCH)}",
            f"BOLTZ_TORCH_INDEX = {json.dumps(_BOLTZ_TORCH_INDEX)}",
            f"BINDSIGHT_GIT = {json.dumps(git_url)}",
        ]
    )
    return header + "\n\n" + _BODY
